...
import { rollDiceGroup, rollExpression, rollParsedExpression, parseRollExpression, ExpressionState, isCriticalFailure, } from "../utils/index.js";
describe("dice rolling functions", () => {
    describe("rollDiceGroup", () => {
        it("should roll basic dice", () => {
            ...
            expect(result.total).toBe(result.rolls.filter(([, , dropped]) => !dropped).reduce((a, [roll]) => a + roll, 0));
        });
    ...
    describe("rollExpression", () => {
        it("should combine multiple dice groups", async () => {
            const expression = {
                diceGroups: [
                    { group: { quantity: 1, sides: 6 }, operator: "+" },
                    { group: { quantity: 0, sides: 3 }, operator: "+" }, // +3 modifier
                ],
            };
        ...
        it("should handle subtraction", async () => {
            const expression = {
                diceGroups: [
                    { group: { quantity: 1, sides: 6 }, operator: "+" },
                    { group: { quantity: 0, sides: 2 }, operator: "-" }, // -2 modifier
                ],
            };
            const result = await rollExpression(expression);
            expect(result.diceGroupResults).toHaveLength(2);
        ...
        it("should roll expression without setting state", async () => {
            const expression = { diceGroups: [{ group: { quantity: 1, sides: 20 }, operator: "+" }] };
            const result = await rollExpression(expression);
