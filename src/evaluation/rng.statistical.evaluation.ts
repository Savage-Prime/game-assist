...
function chiSquareTest(observed: number[], expected: number[]): number {
	...
	for (let i = 0; i < observed.length; i++) {
		...
		if (exp === 0) throw new Error("Expected frequency cannot be zero");
 ...
 * @returns z-score
 */
function runsTest(values: number[]): number {
	let runs = 1;
	...
	const variance =
		(2 * ones * zeros * (2 * ones * zeros - values.length)) / (Math.pow(values.length, 2) * (values.length - 1));
...
describe("RNG Statistical Quality Tests", () => {
	describe("Chi-Square Uniformity Test", () => {
		it("should pass chi-square test for uniform distribution (dice roll simulation)", () => {
			...
			const criticalValue =
				CHI_SQUARE_CRITICAL["0.05"][degreesOfFreedom as keyof (typeof CHI_SQUARE_CRITICAL)["0.05"]] || 11.07;
		...
		it("should pass chi-square test for uniform distribution (d20 simulation)", () => {
			...
			const degreesOfFreedom = bins - 1;
			const criticalValue =
				CHI_SQUARE_CRITICAL["0.05"][degreesOfFreedom as keyof (typeof CHI_SQUARE_CRITICAL)["0.05"]] || 30.144;
