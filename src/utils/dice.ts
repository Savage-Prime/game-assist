...
export async function rollParsedExpression(parsed: RollSpecification, rawExpression?: string): Promise<FullRollResult> {
	...
	const expressionResults = await Promise.all(
		parsed.expressions.map((expression: { diceGroups: Array<{ group: DiceGroup; operator: "+" | "-" }> }) =>
...
export function rollParsedTraitExpression(parsed: TraitSpecification, rawExpression?: string): FullTraitResult {
	// Roll the trait die
